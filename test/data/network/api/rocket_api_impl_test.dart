...
void main() {
  test("getRockets SHOULD return Success WHEN api succeed", () async {
    RocketApiImpl sut = RocketApiImpl(Client());

    ApiState<List<ApiRocket>> apiState = await sut.getRockets();
  ...
  test("getRockets SHOULD return Failure with API_ERROR_CODE_UNKNOWN WHEN api return invalid response", () async {
    MockClient mockClient = MockClient();
  ...
  test("getRockets SHOULD return Failure with API_ERROR_CODE_NETWORK WHEN api return invalid response", () async {
    MockClient mockClient = MockClient();
